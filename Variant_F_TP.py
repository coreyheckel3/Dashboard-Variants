        ...
        dcc.Tab(label='Instructions', children=[
            html.Div([
                ...
                dcc.Markdown('''
                    ...
                    > ###### &#8226 Your budget is capped at $14,000 \n
                    > ###### &#8226 The entire system cannot weigh more than 40,000 kg (for mobility purposes) \n
                    > ###### &#8226 The rocks/projectiles must be able to clear a 200-meter high barrier \n
                    > ###### &#8226 The rocks/projectiles must be able to hit a target at least 800 meters away, but preferably much farther \n
                    > ##### Other than those requirements, your goal is to **maximize the horizontal target distance** that you can reach. Please read all of the following steps before moving on.
        ...
        dcc.Tab(label='Catapult Design Creator', children=[
            ...
            html.Div([  # row
                ...
                html.Div([  # row
                    ...
                    ], className='three columns', style={'display': 'none'}),
                    # dcc.Markdown('##### **Catapult Drawing**'),
                    # dcc.Graph(id='catapult_model',config={'displayModeBar': False},style={'height': '25vh','width': '1hw'}),
                    dcc.Markdown('##### **Projectile Trajectory**'),
                    dcc.Graph(id='projectile_graph', config={'displayModeBar': False},
                              style={'display': 'inline-block', 'height': '300px', 'width': '110%'}),
                ...
                html.Div([  # column
                    ...
                    dbc.Table(
                        ...
                        [html.Tbody([
                            html.Tr([html.Td(['Distance (min 800 m)']), html.Td(id='MD')]),
                            html.Tr([html.Td(['Height (min 200 m)']), html.Td(id='MH')]),
                            html.Tr([html.Td(["Cost (max $14,000 USD)"]), html.Td(id='TC')]),
                            html.Tr([html.Td(["Weight (max 40,000 kg)"]), html.Td(id='WC')])
                        ])],
                ...
                html.Div(id='stop_watch',
                         style={'display': 'none', 'height': '15%', 'width': '125%'}),
                dcc.Interval(
                    id='time-interval',
            ...
            html.Div([
                ...
                html.Button(id='loading-icon', n_clicks=0,
                            children=[html.Div(
                                html.A('SUBMIT', id='download-link', download='data.csv', href="", target="_blank"))],
...
@app.callback(
    Output('projectile_graph', 'figure'),
    [Input('projectile_mass', 'value'),
     Input('release_angle', 'value'),
     Input('arm_length', 'value'),
     Input('spring_constant', 'value')]
)
def update_figure(projectile_mass, release_angle, arm_length, spring_constant):
    h = 0  # mangonel base height
    sm = 5
    global al, k, a1, mass
    al = arm_length  # arm length (m)
    ...
    k = spring_constant  # spring constant (N*m/rad)
    ai = np.deg2rad(0)  # intial angle (deg)
    a0 = np.deg2rad(release_angle)  # release angle (deg)
    a1 = release_angle  # release angle (deg)
    g = 9.81  # gravitational acceleration (m/s^2)
    h0 = np.sin(a0) * al
    ...
    atot = np.deg2rad(90) - a0  # total angle of travel (rad)
    Itot = ((mass ** 2) * al ** 2) + (am * al ** 2) / 3  # total moment of inertia for arm and shot (kg*m^2)
    # rad_sphere = ((3*mass)/(4*np.pi*0.74))**(-1/3)
    # Ball_Itot =  5*(2/5)*mass*rad_sphere

    w = np.sqrt((k * atot ** 2) / (Itot))  # angular velocity at release (rad/s)
    v0 = w * al ** 2  # initial velocity for exterior ballistics (m/s)
    # #exterior ballistics
    t_flight = (v0 * np.sin(a0)) / g  # time of flight (s)
    h = (v0 ** 2 * np.sin(a0) ** 2) / (2 * g)  # max height (m)
    r = (v0 ** 2 * np.sin(2 * a0)) / g  # range (m)
    ...
    # v = np.sqrt(np.square(vx[0]) + np.square(vy[0]))

    x = [0]
    y = [0]

    dragForce = 0.5 * dragCoef * areaProj * (v0 ** 2) * airDen
    # liftForce = 0.5*liftCoef*areaProj*(v0**2)*airDen

    ax = 15 * [(-(dragForce * np.cos(release_angle / 180 * np.pi)) / mass)]
    ay = 15 * [(-grav - (dragForce * np.sin(release_angle / 180 * np.pi) / mass))]
    # ax = [-(((dragForce*np.cos(angle/180*np.pi)+(liftForce*np.cos(angle/180*np.pi)))/mass))]
    # ay = [-grav-(((dragForce*np.sin(angle/180*np.pi)+(liftForce*np.sin(angle/180*np.pi)))/mass))]
    ...
    return {
        ...
        'layout': go.Layout(
            xaxis={'range': [0, 1500],
                   'title': 'Distance Traveled(m)'},
            yaxis={'range': [0, 500],
                   'title': 'Height of Projectile(m)'},
            margin=dict(t=10),
            hovermode='closest'
        )
    }
...
#                 },
#             'shapes': [
#                 {'type': 'rect', 'x0': -18, 'y0': 4, 'x1': 6, 'y1': 12, 'fillcolor': 'rgb(164,116,73)','line': dict(color='none',width=0)},
#                 {'type': 'path', 'path':arm_string, 'fillcolor': 'rgb(164,116,73)', 'line': dict(color='none',width=0)},
#                 {'type': 'circle', 'xref': 'x', 'yref': 'y', 'x0': arm_centerline[1]-wdia_1, 'y0': arm_centerline[3]-wdia_1, 'x1': arm_centerline[1]+wdia_1, 'y1': arm_centerline[3]+wdia_1, 'fillcolor': 'blue', 'line': dict(color='none',width=0)},
#                 {'type': 'rect', 'x0': -18, 'y0': 1, 'x1': -12, 'y1': 5, 'fillcolor':'rgb(164,116,73)', 'line': dict(color='none',width=0)},
#                 {'type': 'circle', 'xref': 'x', 'yref': 'y', 'x0': 0-wdia-wdia_2, 'y0': 3-wdia-wdia_2, 'x1': 0+wdia+wdia_2, 'y1': 5+wdia+wdia_2, 'fillcolor': 'black','line': dict(color='none',width=0)},
#         ]}}
